...
class Upsample(nn.Module):
    def __init__(self, in_channels, out_channels, scale_factor=2):
        ...
        self.upsample = nn.Sequential(
            ...
            # nn.Upsample(scale_factor=scale_factors, mode='bilinear'),
            ConvTranspose2d(
                in_channels,
                out_channels,
                4,
...
class ASFF_3(nn.Module):
    ...
    def forward(self, input1, input2, input3):
        ...
        levels_weight_v = torch.cat(
            (level_1_weight_v, level_2_weight_v, level_3_weight_v), 1)
        levels_weight = self.weight_levels(levels_weight_v)
        levels_weight = F.softmax(levels_weight, dim=1)
...
class ScaleBlockBody(nn.Module):
    def __init__(self, channels=[128, 256, 512]):
        ...
        self.upsample_mid1_2 = Upsample(
            channels[1], channels[0], scale_factor=2)
        ...
        self.upsample_mid2_2 = Upsample(
            channels[1], channels[0], scale_factor=2)
        self.upsample_bot2_2 = Upsample(
            channels[2], channels[1], scale_factor=2)
        self.upsample_bot2_4 = Upsample(
            channels[2], channels[0], scale_factor=4)
    ...
    def forward(self, x):
        ...
        top = self.asff_top2(x1, self.upsample_mid2_2(x2),
                             self.upsample_bot2_4(x3))
        mid = self.asff_mid2(self.downsample_top2_2(x1),
                             x2, self.upsample_bot2_2(x3))
        bot = self.asff_bot2(self.downsample_top2_4(
            x1), self.downsample_mid2_2(x2), x3)
...
class YOLOv8AFPNAA(nn.Module):
    def __init__(self, widen_factor=1, in_channels=[256, 512, 1024], out_channels=[256, 512, 1024],):
        ...
        self.body = nn.Sequential(
            ScaleBlockBody(
                [in_channels[0] // 4, in_channels[1] // 4, in_channels[2] // 4])
        )
